...
- lib — helper library. EVERY lib method is async — you MUST \`await\` every call
  (e.g. \`await lib.result(...)\`, \`events: [await lib.setEvent(...)]\`, and for
  chained access \`(await lib.get(id)).properties\`). Forgetting await is a bug.
${libLines}
...
Use await lib.carried() to check what the player is carrying, then filter by tag. The "Tags in World" section lists all tags currently in use.
</tags>

<code-examples>
Light a candle (requires a flame source in inventory):
\`\`\`
var carried = await lib.carried();
var flameSrc = carried.filter(function(e) { return e.tags.includes("flame-source"); });
if (flameSrc.length === 0) {
  return await lib.result("You have nothing to light it with.");
}
if (object.properties.lit) {
  return await lib.result("The candle is already lit.");
}
return {
  output: "You strike the " + await lib.ref(flameSrc[0]) + " and light the candle.",
  events: [await lib.setEvent(object.id, { property: "lit", value: true, description: "Candle lit" })]
};
\`\`\`
...
\`\`\`
return {
  output: "You eat the " + await lib.ref(object) + ". Not bad!",
  events: [await lib.moveEvent(object.id, { to: "void", from: object.location, description: "Food consumed" })]
};
\`\`\`
...
if (object.properties.switchedOn) {
  return await lib.result("The lantern flickers as you shake it.");
}
return await lib.result("The lantern rattles. You hear liquid sloshing inside.");
\`\`\`

Use one object on another (ditransitive — object + indirect):
\`\`\`
if (!indirect) {
  return await lib.result("Use it on what?");
}
if (!indirect.tags.includes("device") || indirect.properties.powered) {
  return await lib.result("That doesn't seem to help.");
}
return {
  output: "You connect the " + await lib.ref(object) + " to the " + await lib.ref(indirect) + ". It hums to life.",
  events: [await lib.setEvent(indirect.id, { property: "powered", value: true, description: "Powered on" })]
};
\`\`\`
...
if (object.description.includes("stripped")) {
  return await lib.result("There is nothing left to strip.");
}
return {
  output: "You strip the copper wire from the " + await lib.ref(object) + ".",
  events: [
    await lib.setEvent(object.id, { property: "description", value: "The device has been stripped of its copper wiring.", description: "Stripped" }),
    await lib.createEvent("item:copper-wire", { tags: ["portable"], properties: { name: "Copper Wire", description: "A tangle of salvaged copper wire.", location: player.id }, description: "Created copper wire" })
  ]
};
\`\`\`
</code-examples>
...
Entity creation: await lib.createEvent(entityId, { tags, properties, description }). Use "category:slug" format for IDs (e.g. "item:copper-wire"). Set location to player.id to give it to the player, or room.id to place it in the room.
</events>
...
- When an action reveals or discovers something (a hidden compartment, a feature, a component), CREATE it as an entity using await lib.createEvent so the player can interact with it. Don't just describe it in text — if the player can see it, they should be able to touch it.
</guidelines>`;
}
